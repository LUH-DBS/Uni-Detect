    ...
    try:
        ...
        with Pool() as pool:
            map_results = pool.map(
                mapper,
                [(col_id, se_dict[col_id], shared_args) for col_id in se_dict.keys()],
            )
        ...
        if ground_truth:
            ...
            dirty_value = test_df[test_df.columns[test_column_idx]].values.astype(str)[
                idx_p
            ]
        else:

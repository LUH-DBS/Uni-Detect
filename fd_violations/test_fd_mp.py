...
def process_pair(test_df, pair, path, fd_dict, tokens_dict, ground_truth, config):
    test_column_1, test_column_2, left_ness_1, left_ness_2, fd_d, fd_do, idx_d = (
        np.nan,
        np.nan,
        np.nan,
        np.nan,
    ...
    if idx_d != -1 and fd_do != fd_d:
        ...
        str_col_1 = test_column_1.astype(str)
        str_col_2 = test_column_2.astype(str)
        test_col_1_dtype = (
            "alnumeric" if str_col_1.str.isalnum().all() else test_column_1.dtype
        )
        test_col_2_dtype = (
            "alnumeric" if str_col_2.str.isalnum().all() else test_column_2.dtype
        )
        for cols_id in fd_dict.keys():
            if (
                fd_dict[cols_id]["d_type_1"] != test_col_1_dtype
                or fd_dict[cols_id]["d_type_2"] != test_col_2_dtype
            ):
                continue
            if fd_dict[cols_id]["number_of_rows_range"] != number_of_rows_range:
                continue
            if (
                fd_dict[cols_id]["left_ness_1"] != left_ness_1
                or fd_dict[cols_id]["left_ness_2"] != left_ness_2
            ):
                continue
            if (
                fd_dict[cols_id]["avg_col_pre_1"] != avg_col_pre_1
                or fd_dict[cols_id]["avg_col_pre_2"] != avg_col_pre_2
            ):
                continue
        ...
        if lr != -np.inf:
            if idx_d != -1:
                if ground_truth:
                    ...
                    correct_value_1 = clean_df[
                        clean_df.columns[clean_col_idx_0]
                    ].values.astype(str)[idx_d]
                    correct_value_2 = clean_df[
                        clean_df.columns[clean_col_idx_1]
                    ].values.astype(str)[idx_d]
                else:
                ...
                row = [
                    ...
                    pair[1],
                    idx_d,
                    list(test_df.columns).index(pair[0]),
                    list(test_df.columns).index(pair[1]),
                    lr,
                    test_column_1.loc[idx_d],
                    test_column_2.loc[idx_d],
                    correct_value_1,
                    correct_value_2,
                    str(correct_value_1) != str(dirty_value_1)
                    or str(correct_value_2) != str(dirty_value_2),
                ]
                rows.append(row)
    return rows
...
def process_table(
    ...
    try:
        ...
        with ThreadPoolExecutor() as executor:
            pairs = combinations(test_df.columns, 2)
            for pair in pairs:
                futures.append(
                    executor.submit(
                        process_pair,
...
def main():
    ...
    logging.info(f"FD Test finished in {t_f} seconds")
    return

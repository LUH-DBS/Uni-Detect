...
def process_pair(test_df, pair, path, fd_dict, tokens_dict, ground_truth, config):
    test_column_1, test_column_2, left_ness_1, \
    left_ness_2, fd_d, fd_do, idx_d = np.nan, np.nan, np.nan, np.nan,\
    ...
    if idx_d != -1 and fd_do != fd_d:
        ...
        test_col_1_dtype = "alnumeric" if str_col_1.str.isalnum().all() else test_column_1.dtype
        test_col_2_dtype = "alnumeric" if str_col_2.str.isalnum().all() else test_column_2.dtype
        for cols_id in fd_dict.keys():
            if fd_dict[cols_id]["d_type_1"] != test_col_1_dtype or\
                    fd_dict[cols_id]["d_type_2"] != test_col_2_dtype:
                continue
...
def process_table(file_type, path, fd_dict, tokens_dict, ground_truth, fd_results, tables_output_path, config):
    ...
    try:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for pair in pairs:
                futures.append(executor.submit(process_pair, test_df, pair, path, fd_dict, tokens_dict, ground_truth, config))
...
def main():
    ...
    t_init = time.time()
    for path in test:
        fd_results = process_table(file_type, path, fd_dict, tokens_dict, ground_truth, fd_results, tables_output_path, config)

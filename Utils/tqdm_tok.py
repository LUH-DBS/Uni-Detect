...
def get_dict(tokens_dir_path, output_path):
    ...
    with Pool() as pool:
        results = list(
            tqdm(
                pool.imap(count_tokens, td),
                total=len(td),
                desc="Processing tokens",
                unit="table",
            )
        )
    aggregated_tokens_counter = tqdm(
        sum(results, Counter()), desc="Aggregating tokens", unit="token"
    )
    print(f"Finish aggregating tokens dict")
    tokens_dict = {
        k: v / n_tables
        for k, v in tqdm(
            aggregated_tokens_counter.items(), desc="Creating tokens dict", unit="item"
        )
    }
...
if __name__ == "__main__":
    get_dict(
        ...
        "/home/fatemeh/EDS-BaseLines/Uni-Detect/Utils",
    )
